...
TOKEN = 'MTUxMjg3NDU5MTUyMzk2MzAwMA.GgkoTp.Jiec8tHK_GKi8fWzCB99NniLk0gnQhySL3FljA' 
ADMIN_ID = 1284107691723067454 
EXTRA_ROLE_ID = 1512850411554345030 
GACHA_COST = 1000 

# --- 🎭 ตั้งค่า Rank (ครบถ้วน 1-255) ---
GROUPS_CONFIG = [
    {
        "group_id": 35646818,
        "ranks": {
...
def load_db():
    ...
    for user_id, user_data in users_db.items():
        ...
        if "last_vc_join_time" not in user_data: user_data["last_vc_join_time"] = None
...
async def perform_sync_silent(guild, discord_id, roblox_username):
    try:
        ...
        first_group_id = GROUPS_CONFIG[0]['group_id']
        rank_id = user_groups.get(first_group_id, 0)
...
class VerificationModal(discord.ui.Modal, title='ยืนยันตัวตน Roblox'):
    username = discord.ui.TextInput(label='ชื่อใน Roblox')
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            active_sessions[str(interaction.user.id)] = {
                "roblox_username": u_name, "roblox_user_id": user_id, 
                "verification_code": code, "time": datetime.datetime.now().strftime("%H:%M:%S")
            }
            await interaction.followup.send(f"รหัสของคุณคือ: **{code}**\nใส่ในเกมแล้วกลับมากดอัปเดตยศครับ", ephemeral=True)
...
class ManagementView(discord.ui.View):
    ...
    async def update(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send("✅ อัปเดตยศเรียบร้อย!", ephemeral=True)
    @discord.ui.button(label="🔑 เปลี่ยนไอดี Roblox", style=discord.ButtonStyle.secondary)
...
class VerifyView(discord.ui.View):
    ...
    async def verify(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if d_id in users_db:
            await interaction.response.send_message(embed=discord.Embed(title="เมนูจัดการ", description=f"ผูกกับไอดี: **{users_db[d_id]['roblox_username']}**"), view=ManagementView(users_db[d_id]['roblox_username']), ephemeral=True)
        else: await interaction.response.send_modal(VerificationModal())

# --- 🎰 แผงกาชา ---
class GachaView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="เช็ค EXP", style=discord.ButtonStyle.primary, emoji="📊", custom_id="g_check_xp")
    async def check_xp(self, interaction: discord.Interaction, button: discord.ui.Button):
        d_id = str(interaction.user.id)
        xp = users_db.get(d_id, {}).get("xp", 0)
        await interaction.response.send_message(f"✨ EXP ของคุณคือ: `{xp:,}`", ephemeral=True)
    @discord.ui.button(label="สุ่ม Role", style=discord.ButtonStyle.success, emoji="🎲", custom_id="g_roll")
    async def roll_role(self, interaction: discord.Interaction, button: discord.ui.Button):
        d_id = str(interaction.user.id)
        ...
        save_db()
        await interaction.response.send_message("⏳ กำลังสุ่ม...", ephemeral=True)
        await asyncio.sleep(1.5)
        # โอกาส 35% ร้อยตำรวจตรี | 65% เกลือ
        outcome = random.choices(["Salt", "Reward"], weights=[65, 35], k=1)[0]
        if outcome == "Salt": await interaction.edit_original_response(content="🧂 เกลือจ้า! พยายามใหม่นะ")
        else: await interaction.edit_original_response(content="🎖️ ยินดีด้วย! คุณได้รับยศ **ร้อยตำรวจตรี**\nโปรดติดต่อแอดมินเพื่อรับยศครับ!")

# --- 👑 คำสั่ง Admin ---
@bot.tree.command(name="sync_commands", description="ล้างและรีเฟรชคำสั่งทั้งหมด (Admin)")
...
async def sync_commands(interaction: discord.Interaction):
    ...
    await bot.tree.sync()
    await interaction.followup.send("✅ รีเฟรชคำสั่งสำเร็จ!", ephemeral=True)

@bot.tree.command(name="add_xp", description="เพิ่ม EXP ให้สมาชิก (Admin)")
@app_commands.check(is_admin)
async def add_xp(interaction: discord.Interaction, member: discord.Member, amount: int):
    m_id = str(member.id)
    if m_id not in users_db: users_db[m_id] = {"xp": 0, "last_vc_join_time": None}
    users_db[m_id]["xp"] += amount
    save_db()
    await interaction.response.send_message(f"✅ เพิ่ม `{amount:,}` EXP ให้ {member.mention}", ephemeral=True)

@bot.tree.command(name="check_pending", description="ดูรายชื่อรอยืนยัน (Admin)")
@app_commands.check(is_admin)
async def check_pending(interaction: discord.Interaction):
    if not active_sessions: return await interaction.response.send_message("📭 ไม่มีรายการรอยืนยัน", ephemeral=True)
    msg = "📋 **รายการรอยืนยัน:**\n"
    for d_id, data in active_sessions.items():
        msg += f"- <@{d_id}> | Roblox: `{data['roblox_username']}` | Code: `{data['verification_code']}`\n"
    await interaction.response.send_message(msg, ephemeral=True)

@bot.tree.command(name="clear_pending", description="ล้างรายการรอยืนยัน (Admin)")
@app_commands.check(is_admin)
async def clear_pending(interaction: discord.Interaction, member: discord.Member):
    m_id = str(member.id)
    if m_id in active_sessions:
        del active_sessions[m_id]
        await interaction.response.send_message(f"✅ ล้างรายการของ {member.mention} แล้ว", ephemeral=True)
    else: await interaction.response.send_message(f"❌ ไม่พบรายการ", ephemeral=True)

@bot.tree.command(name="setup_verify", description="ติดตั้งแผงยืนยันตัวตน (Admin)")
@app_commands.check(is_admin)
async def setup_verify(interaction: discord.Interaction):
    await interaction.channel.send(embed=discord.Embed(title="ระบบยืนยันตัวตน", color=discord.Color.blue()), view=VerifyView())
    await interaction.response.send_message("ติดตั้งสำเร็จ", ephemeral=True)

@bot.tree.command(name="setup_gacha", description="ติดตั้งแผงกาชา (Admin)")
@app_commands.check(is_admin)
async def setup_gacha(interaction: discord.Interaction):
    embed = discord.Embed(title="🎮 ระบบ สุ่มยศ และ เช็ค EXP", description=f"ใช้ `{GACHA_COST:,}` EXP ต่อการสุ่ม\n\n🎖️ **ร้อยตำรวจตรี** (35%)\n🧂 **เกลือ** (65%)", color=discord.Color.gold())
...
async def on_ready():
    ...
    bot.add_view(VerifyView()); bot.add_view(GachaView())
    xp_task.start()
...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        if str(member.id) in users_db:
            users_db[str(member.id)]["last_vc_join_time"] = datetime.datetime.now().isoformat()
            save_db()
    elif before.channel is not None and after.channel is None:
        if str(member.id) in users_db and users_db[str(member.id)]["last_vc_join_time"] is not None:
            join_time = datetime.datetime.fromisoformat(users_db[str(member.id)]["last_vc_join_time"])
            minutes = int((datetime.datetime.now() - join_time).total_seconds() / 60)
            users_db[str(member.id)]["xp"] += minutes * 35
            users_db[str(member.id)]["last_vc_join_time"] = None
            save_db()
...
async def xp_task():
    for guild in bot.guilds:
        for member in guild.members:
            ...
            if member.voice and member.voice.channel and str(member.id) in users_db and users_db[str(member.id)]["last_vc_join_time"] is not None:
                users_db[str(member.id)]["xp"] += 35
                users_db[str(member.id)]["last_vc_join_time"] = datetime.datetime.now().isoformat()
                save_db()

@xp_task.before_loop
async def before_xp_task(): await bot.wait_until_ready()
...
def complete_verification():
    ...
    username = data.get("roblox_username", "").strip()
    code = data.get("verification_code", "").strip()
    for d_id, session in active_sessions.items():
        if session["roblox_username"].lower() == username.lower() and session["verification_code"] == code:
            users_db[d_id] = {"roblox_username": session["roblox_username"], "roblox_user_id": session["roblox_user_id"], "status": "verified", "xp": users_db.get(d_id, {}).get("xp", 0), "last_vc_join_time": None}
            save_db(); del active_sessions[d_id]
            if bot.guilds: bot.loop.create_task(perform_sync_silent(bot.guilds[0], d_id, session["roblox_username"]))
