...
class VerificationModal(discord.ui.Modal, title='ยืนยันตัวตน Roblox'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            r_id = res_user["data"][0]["id"]
            code = str(uuid.uuid4())[:8]
            active_sessions[str(interaction.user.id)] = {"roblox_username": u_name, "roblox_id": r_id, "verification_code": code}
...
class ManagementView(discord.ui.View):
    ...
    async def update_rank(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if success: await interaction.followup.send("✅ อัปเดตยศและชื่อเรียบร้อยแล้ว!", ephemeral=True)
        else: await interaction.followup.send("❌ เกิดข้อผิดพลาดในการอัปเดต", ephemeral=True)
...
class VerifyView(discord.ui.View):
    ...
    async def verify_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if d_id in users_db:
            u_data = users_db[d_id]
            username = u_data.get("roblox_username", "Unknown")
            roblox_id = u_data.get("roblox_id", "ไม่มีข้อมูล")
            embed = discord.Embed(
                title="⚠️ พบข้อมูล Roblox Account อยู่แล้ว",
                description=f"**ข้อมูลปัจจุบัน:**\n> Roblox: **{username}**\n> Roblox ID: `{roblox_id}`\n> สถานะ: ✅ ยืนยันแล้ว\n\n💡 **ต้องการเปลี่ยน Account? กดปุ่มด้านล่าง**",
