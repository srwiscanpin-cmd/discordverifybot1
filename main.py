...
# --- ⚙️ Configuration ---
# รองรับทั้งชื่อ DISCORD_TOKEN และ BOT_TOKEN เพื่อป้องกันความผิดพลาด
...
if not TOKEN:
    print("❌ Error: ไม่พบ Token ใน Environment Variables! กรุณาเช็คใน Railway")
...
class VerifyView(discord.ui.View):
    ...
    @discord.ui.button(label="📝 ยืนยันตัวตน", style=discord.ButtonStyle.success, custom_id="btn_verify_v7")
    async def verify_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        d_id = str(interaction.user.id)
        if d_id in users_db:
            username = users_db[d_id].get("roblox_username", "Unknown")
            embed = discord.Embed(title="✅ ยืนยันตัวตนแล้ว", description=f"เชื่อมต่อกับไอดี: **{username}**", color=discord.Color.green())
            await interaction.response.send_message(embed=embed, ephemeral=True)
        else: await interaction.response.send_modal(VerificationModal())

class GachaView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="📊 เช็ค EXP", style=discord.ButtonStyle.primary, custom_id="btn_xp_v7")
    ...
    @discord.ui.button(label="🎲 สุ่ม Role", style=discord.ButtonStyle.success, custom_id="btn_roll_v7")
