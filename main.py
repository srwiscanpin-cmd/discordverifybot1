...
# --- 🎭 Rank & Group Config (ใส่ให้ครบตามที่คุณต้องการ) ---
GROUPS_CONFIG = [
    {
        "group_id": 35646818,
        "ranks": {
...
def load_db():
    ...
    for uid in users_db:
        if "xp" not in users_db[uid]: users_db[uid]["xp"] = 0
...
class VerifyView(discord.ui.View):
    ...
    @discord.ui.button(label="📝 ยืนยันตัวตน", style=discord.ButtonStyle.success, custom_id="persistent_verify_btn")
    async def verify_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        d_id = str(interaction.user.id)
        if d_id in users_db:
            await interaction.response.send_message(f"ผูกกับไอดี: **{users_db[d_id]['roblox_username']}**", view=ManagementView(users_db[d_id]['roblox_username']), ephemeral=True)
        else: await interaction.response.send_modal(VerificationModal())

class GachaView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="เช็ค EXP", style=discord.ButtonStyle.primary, custom_id="persistent_xp_btn")
    async def xp_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(f"✨ EXP: `{xp:,}`", ephemeral=True)
    @discord.ui.button(label="สุ่ม Role", style=discord.ButtonStyle.success, custom_id="persistent_roll_btn")
    async def roll_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        save_db()
        outcome = random.choices(["Salt", "Reward"], weights=[65, 35])[0]
        await interaction.response.send_message("🧂 เกลือจ้า!" if outcome == "Salt" else "🎖️ ยินดีด้วย! ได้รับยศ **ร้อยตำรวจตรี**", ephemeral=True)
...
async def setup_verify(interaction: discord.Interaction):
    if interaction.user.id != ADMIN_ID: return await interaction.response.send_message("❌ No Permission", ephemeral=True)
...
async def setup_gacha(interaction: discord.Interaction):
    if interaction.user.id != ADMIN_ID: return await interaction.response.send_message("❌ No Permission", ephemeral=True)
    embed = discord.Embed(title="🎰 ระบบสุ่มยศ", description=f"ค่าสุ่ม: `{GACHA_COST:,}` EXP", color=discord.Color.gold())
...
app = Flask(__name__)
@app.route('/')
def home(): return "Bot API is Online"

@app.route("/complete_verification", methods=["POST"])
def complete_verification():
    data = flask_request.json
    username, code = data.get("roblox_username"), data.get("verification_code")
    for d_id, sess in active_sessions.items():
        if sess["roblox_username"] == username and sess["verification_code"] == code:
            users_db[d_id] = {"roblox_username": username, "xp": users_db.get(d_id, {}).get("xp", 0)}
            save_db(); del active_sessions[d_id]
            if bot.guilds: bot.loop.create_task(perform_sync_silent(bot.guilds[0], d_id, username))
